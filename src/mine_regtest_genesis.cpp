...
std::array<uint8_t, 32> createMerkleRoot(const std::string& timestamp) {
    ...
    std::string data = "Coinbase transaction for Paperclips regtest: " + timestamp;
...
int main() {
    // Regtest Genesis Block Parameters - very easy difficulty
    const uint32_t timestamp = 1740528000; // Feb 28, 2025
    const uint32_t bits = 0x207fffff; // Regtest difficulty - very easy
    ...
    // For regtest, we'll just use nonce=2 as in the original Bitcoin regtest
    header.nNonce = 2;
    
    std::cout << "Paperclips Regtest Genesis Block" << std::endl;
