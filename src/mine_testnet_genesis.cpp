...
struct BlockHeader {
    ...
    bool meetsTarget() const {
        ...
        // For testnet mining, we'll look for a hash that starts with 3 zeros
        // This is much easier than the actual difficulty
...
std::array<uint8_t, 32> createMerkleRoot(const std::string& timestamp) {
    ...
    std::string data = "Coinbase transaction: " + timestamp;
...
int main() {
    // Testnet Genesis Block Parameters
    const uint32_t timestamp = 1740528000; // Feb 28, 2025
    const uint32_t bits = 0x1e0ffff0; // Testnet difficulty
    ...
    // Status tracking
    uint32_t startNonce = 1234567; // Start with our placeholder nonce
    header.nNonce = startNonce;
    uint32_t hashesComputed = 0;
    auto startTime = std::chrono::steady_clock::now();
    
    std::cout << "Paperclips Testnet Genesis Block Miner" << std::endl;
    ...
    if (found) {
        ...
        std::cout << "Failed to find a valid genesis block." << std::endl;
    }
