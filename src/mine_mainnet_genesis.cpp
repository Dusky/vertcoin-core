...
struct BlockHeader {
    ...
    bool meetsTarget() const {
        ...
        // For mainnet, look for a hash that starts with 4+ zeros
...
std::array<uint8_t, 32> createMerkleRoot(const std::string& timestamp) {
    ...
    std::string data = "Coinbase transaction for Paperclips mainnet: " + timestamp;
...
int main() {
    // Mainnet Genesis Block Parameters
    const uint32_t timestamp = 1740528000; // Feb 28, 2025
    const uint32_t bits = 0x1e0ffff0; // Mainnet difficulty
    ...
    // Status tracking
    uint32_t startNonce = 2083236893; // Start with our placeholder nonce
    header.nNonce = startNonce;
    uint32_t hashesComputed = 0;
    auto startTime = std::chrono::steady_clock::now();
    
    std::cout << "Paperclips Mainnet Genesis Block Miner" << std::endl;
